...
async fn create_inner(
	...
	polyio::write(cluster_path.join(oneclient_common::paths::DEDICATED_MARKER), b"").await.ok();
...
async fn ensure_content_dirs(cluster_path: &std::path::Path) -> ClusterResult<()> {
	...
	let options_path = cluster_path.join("options.txt");
	if !options_path.exists() {
		let _ = polyio::write(&options_path, b"soundCategory_music:0.0\nsoundCategory_record:0.0\n").await;
	}
	Ok(())
}
