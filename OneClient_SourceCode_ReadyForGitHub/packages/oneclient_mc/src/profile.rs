...
pub async fn fetch_player_profile(
    ...
    let is_raw_uuid = (trimmed.len() == 32 || (trimmed.len() == 36 && trimmed.contains('-')))
        && trimmed.chars().all(|c| c.is_ascii_hexdigit() || c == '-');

    let resolved_uuid = if is_raw_uuid {
        normalize_uuid(trimmed).to_string()
    } else {
        #[derive(Deserialize)]
        struct UsernameResponse {
            id: String,
        }
        let uname_url = format!("https://api.mojang.com/users/profiles/minecraft/{trimmed}")
            .parse()
            .map_err(McError::Url)?;
        match client.send_as::<UsernameResponse>(reqwest::Request::new(Method::GET, uname_url)).await {
            Ok(res) => res.id,
            Err(_) => normalize_uuid(trimmed).to_string(),
        }
    };

    let url = format!("https://sessionserver.mojang.com/session/minecraft/profile/{resolved_uuid}")
        .parse()
        .map_err(McError::Url)?;
    ...
    let texture_property = response
        ...
        .ok_or_else(|| McError::Minecraft("no textures in player profile".into()))?;
...
pub async fn fetch_logged_in_profile(
    client: &RequestClient,
    access_token: &str,
) -> McResult<MojangFullPlayerProfile> {
...
mod tests {
    ...
    async fn fresh_cache_entry_is_served_without_network() {
        ...
        let client = RequestClient::new(oneclient_net::NetConfig::default()).unwrap();
