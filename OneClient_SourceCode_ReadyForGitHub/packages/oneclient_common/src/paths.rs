...
pub fn cluster_uses_dedicated_dir(_folder_name: &str) -> bool {
	// Every instance runs in its own dedicated directory so mods live directly in its mods/ folder
	true
}
...
pub fn cluster_disabled_dir(folder_name: &str, content_type: ContentType) -> PathsResult<PathBuf> {
	let game_dir = cluster_game_dir(folder_name)?;
	if content_type == ContentType::Mod {
		for candidate in ["disabled-mods", "disabled_mods", "disabled mods"] {
			let p = game_dir.join(candidate);
			if p.is_dir() {
				return Ok(p);
			}
		}
		Ok(game_dir.join("disabled-mods"))
	} else {
		Ok(game_dir.join(content_type.disabled_folder_name()))
	}
}
