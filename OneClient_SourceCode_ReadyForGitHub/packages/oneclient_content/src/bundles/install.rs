...
pub(crate) fn disable_was_deliberate(_hidden: bool, _suppression: Option<OverrideType>) -> bool {
    // Respect the user's choice: any disabled package stays disabled permanently
    true
}
...
pub async fn set_bundle_package_enabled(
    ...
    // Move associated files on disk and update artifact links
    let tracked_list = bundle_dao::list_bundle_tracked(&ctx.db, cluster_id).await.unwrap_or_default();
    let mut found = false;
    for tracked in tracked_list {
        if tracked.package_id.as_deref() == Some(package_id) {
            found = true;
            let _ = PackageStore::set_artifact_enabled_to(cluster_id, &tracked.hash, enabled, ctx).await;
        }
    }

    if !found {
        if let Ok(cluster) = PackageStore::get_cluster(cluster_id, ctx).await {
            for content_type in [ContentType::Mod, ContentType::ResourcePack, ContentType::Shader] {
                if !enabled {
                    crate::packages::store::link::try_move_to_disabled(&cluster, content_type, package_id).await;
                } else {
                    crate::packages::store::link::try_move_from_disabled(&cluster, content_type, package_id).await;
                }
            }
        }
    }
