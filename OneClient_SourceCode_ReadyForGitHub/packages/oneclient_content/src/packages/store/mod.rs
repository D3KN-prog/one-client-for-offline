...
pub use link::{link_or_copy, remove_entry, try_unlink_materialized, try_move_to_disabled, try_move_from_disabled};
...
impl PackageStore {
    ...
    pub async fn link_artifact(
        ...
        // Materialize immediately into the cluster's folder so it's visible in the mods/ directory right away!
        if let Ok(root) = oneclient_common::paths::cluster_dir(&cluster.folder_name) {
            let content_type = ContentType::from_repr(artifact.content_type as u8)
                .unwrap_or(ContentType::Mod);
            let dest_dir = root.join(content_type.folder_name());
            polyio::create_dir_all(&dest_dir).await.ok();
            let dest = dest_dir.join(name);
            if let Ok(src) = paths::artifact_absolute_path(&artifact.path) {
                if src.exists() {
                    let _ = link::link_or_copy(&src, &dest).await;
                }
            }
        }
    ...
    async fn write_artifact_enabled(
        ...
        let link = match artifact_dao::get_cluster_artifact(&ctx.db, cluster_id, hash).await? {
            Some(l) => l,
            None => {
                let file_name = std::path::Path::new(&artifact.path)
                    ...
                    .unwrap_or("mod.jar")
                    .to_string();
                let _ = artifact_dao::link_cluster_artifact(&ctx.db, cluster_id, hash, &file_name).await;
                artifact_dao::get_cluster_artifact(&ctx.db, cluster_id, hash)
                    .await?
                    .ok_or(PackageError::ArtifactMissing(hash.to_string()))?
            }
        };
        ...
        if !enabled {
            link::try_move_to_disabled(&cluster, content_type, &link.cluster_file_name).await;
            if link.cluster_file_name != file_name {
                link::try_move_to_disabled(&cluster, content_type, &file_name).await;
            }
        } else {
            let restored = link::try_move_from_disabled(&cluster, content_type, &file_name).await
                || (link.cluster_file_name != file_name && link::try_move_from_disabled(&cluster, content_type, &link.cluster_file_name).await);
            if !restored {
                if let Ok(root) = oneclient_common::paths::cluster_game_dir(&cluster.folder_name) {
                    let dest_dir = root.join(content_type.folder_name());
                    polyio::create_dir_all(&dest_dir).await.ok();
