...
pub async fn import_manual_content(
    ...
    for content_type in SWAP_TYPES {
        ...
        let disabled_dir = oneclient_common::paths::cluster_disabled_dir(&cluster.folder_name, content_type)
            .unwrap_or_else(|_| game_dir.join(content_type.disabled_folder_name()));
        if let Ok(mut dis_entries) = polyio::read_dir(&disabled_dir).await {
            while let Ok(Some(entry)) = dis_entries.next_entry().await {
                let Ok(file_type) = entry.file_type().await else {
                    continue;
                };
                if !file_type.is_file() {
                ...
                match PackageStore::import_local_file(&path, content_type, cluster.id, &services.content()).await {
                    Ok(row) => {
                        let _ = oneclient_db::dao::artifact::update_cluster_artifact(&services.db, cluster.id, &row.hash, name, 0).await;
                        tracing::debug!(file = name, "registered manually-added disabled content");
                    }
                    Err(err) => tracing::warn!(
                        file = name,
                        error = %err,
                        "failed to register manually-added disabled content"
                    ),
                }
            }
        }
...
async fn ensure_note(dir: &Path, _content_type: ContentType) {
    let note = dir.join(EMPTY_NOTE_NAME);
    if polyio::try_exists(&note).await.unwrap_or(false) {
        polyio::remove_file(&note).await.ok();
    }
}
