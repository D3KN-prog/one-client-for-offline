...
pub async fn launch_cluster(
    ...
    let cluster = if existing.stage == ClusterStage::Ready {
        ...
        {
            Ok(cluster) => {
                let has_mods = cluster.game_dir().ok().map(|d| {
                    let m = d.join("mods");
                    m.is_dir() && std::fs::read_dir(&m).map(|mut r| r.next().is_some()).unwrap_or(false)
                }).unwrap_or(false);
    ...
    silence_in_game_music(&cwd).await;
...
async fn silence_in_game_music(game_dir: &Path) {
    let options_path = game_dir.join("options.txt");
    match polyio::read_to_string(&options_path).await {
        Ok(content) => {
            ...
            for line in content.lines() {
                if line.starts_with("soundCategory_music:") {
                    lines.push("soundCategory_music:0.0".to_string());
                    found_music = true;
                } else if line.starts_with("soundCategory_record:") {
                    lines.push("soundCategory_record:0.0".to_string());
                    found_record = true;
                } else {
            ...
            let _ = polyio::write(&options_path, updated.as_bytes()).await;
        }
        Err(_) => {
            let initial = "soundCategory_music:0.0\nsoundCategory_record:0.0\n";
            let _ = polyio::write(&options_path, initial.as_bytes()).await;
        }
    }
}
