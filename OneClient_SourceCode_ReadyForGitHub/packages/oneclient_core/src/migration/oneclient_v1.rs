...
pub fn old_root() -> Option<PathBuf> {
    let base = directories::BaseDirs::new().map(|d| d.data_dir().join("PulseClient"));
...
async fn detect_inner(root: &Path, db_path: &Path) -> LauncherResult<MigrationDetection> {
    ...
    Ok(MigrationDetection {
        source: MigrationSource::PulseClientV1,
...
mod tests {
    ...
    fn category_extraction() {
        assert_eq!(
            category_from_bundle_name("PulseClient 1.21.1 Fabric [HUD]"),
            Some("HUD".to_string())
        );
        assert_eq!(
            category_from_bundle_name("PulseClient 1.21.11 Fabric [QoL]"),
            Some("QoL".to_string())
        );
