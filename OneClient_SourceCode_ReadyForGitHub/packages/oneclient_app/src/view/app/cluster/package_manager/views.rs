...
use crate::hooks::{use_cluster_mutation, use_dispatch, ClusterAction};
...
pub(super) fn toolbar_bar(
    ...
    // Collect hashes of all toggleable items for bulk actions
    let all_hashes: Vec<String> = items
        .iter()
        .filter_map(|p| p.hash.clone())
        .collect();

    let bundle_items: Vec<(String, String, bool, bool)> = items
        .iter()
        .filter(|p| p.in_bundle())
        .filter_map(|p| p.bundle_name.clone().map(|b| (b, p.package_id.clone(), false, p.manifest_default)))
        .collect();

    let all_enabled = items.iter().all(|p| p.enabled);
    ...
    rect()
        ...
        .child(
            Button::new()
                ...
                .font_size(12.)
                .on_press({
                    let bundle_items = bundle_items.clone();
                    ...
                    move |_: Event<PressEventData>| {
                        ...
                        spawn(async move {
                            if let Ok(state) = crate::launcher::state() {
                                if let Ok(Some(row)) = oneclient_db::dao::cluster::get_by_id(&state.services.db, cluster_id).await {
                                    let mut candidates = vec![
                                        oneclient_common::paths::cluster_game_dir(&row.folder_name).ok(),
                                        oneclient_common::paths::cluster_dir(&row.folder_name).ok(),
                                        oneclient_common::paths::shared_minecraft_dir().ok(),
                                    ];
                                    candidates.dedup();
                                    for dir_opt in candidates {
                                        let Some(g_dir) = dir_opt else { continue; };
                                        let active_dir = g_dir.join(content_type.folder_name());
                                        let disabled_dir = oneclient_common::paths::cluster_disabled_dir(&row.folder_name, content_type)
                                            .unwrap_or_else(|_| g_dir.join(content_type.disabled_folder_name()));
                                        let _ = polyio::create_dir_all(&disabled_dir).await;

                                        if let Ok(mut entries) = polyio::read_dir(&active_dir).await {
                                                ...
                                                if let Ok(ft) = entry.file_type().await {
                                                    if ft.is_file() {
                                                        ...
                                                        if let Some(file_name) = path.file_name() {
                                                            ...
                                                            if polyio::copy(&path, &dest).await.is_ok() {
                                    ...
                                    let _ = oneclient_db::dao::artifact::set_all_cluster_artifacts_enabled(
                                        &state.services.db,
                                        cluster_id,
                                        0,
                                    )
                                    ...
                                    for (bundle_name, package_id, _, _) in &bundle_items {
                                        let _ = oneclient_db::dao::cluster_bundle::save_override(
                                            &state.services.db,
                                            cluster_id,
                                            bundle_name,
                                            package_id,
                                            oneclient_db::models::OverrideType::Disabled,
                                        )
                                        .await;
                                    }

                                    let _ = crate::hooks::invalidate_cluster_content_queries().await;
                                }
                            }
                        });
                    }
                            ...
                            if let Ok(state) = crate::launcher::state() {
                                if let Ok(Some(row)) = oneclient_db::dao::cluster::get_by_id(&state.services.db, cluster_id).await {
                                    ...
                                    for dir_opt in candidates {
                                        ...
                                        if let Ok(mut entries) = polyio::read_dir(&disabled_dir).await {
                                            while let Ok(Some(entry)) = entries.next_entry().await {
                                                if let Ok(ft) = entry.file_type().await {
                                                    if ft.is_file() {
                                                        let path = entry.path();
                                                        if let Some(file_name) = path.file_name() {
                                                            let dest = active_dir.join(file_name);
                                                            if polyio::copy(&path, &dest).await.is_ok() {
                                                                let _ = polyio::remove_file(&path).await;
                                                            }
                                                        }
                                    ...
                                    let _ = oneclient_db::dao::cluster_bundle::remove_all_overrides_for_cluster(
                                        &state.services.db,
                                        cluster_id,
                                    )
                                    .await;
        ...
        .child(
            Button::new()
                ...
                .font_size(12.)
                .on_press({
                    let folder_name = content_type.folder_name();
                    move |_| {
                        let cid = cluster_id;
                        spawn(async move {
                            if let Ok(state) = crate::launcher::state() {
                                if let Ok(Some(row)) = oneclient_db::dao::cluster::get_by_id(&state.services.db, cid).await {
                                    if let Ok(dir) = oneclient_common::paths::cluster_dir(&row.folder_name) {
                                        let target = dir.join(folder_name);
                                        let _ = polyio::create_dir_all(&target).await;
                                        crate::platform::open_url(&target.to_string_lossy());
                                    }
                                }
                            }
                        });
                ...
                .on_press({
                    ...
                    move |_| {
                        ...
                        spawn(async move {
                            if let Ok(state) = crate::launcher::state() {
                                if let Ok(Some(row)) = oneclient_db::dao::cluster::get_by_id(&state.services.db, cid).await {
                                    if let Ok(dis_dir) = oneclient_common::paths::cluster_disabled_dir(&row.folder_name, content_type) {
                                        let _ = polyio::create_dir_all(&dis_dir).await;
                                        crate::platform::open_url(&dis_dir.to_string_lossy());
                                    }
                                }
                            }
                        });
