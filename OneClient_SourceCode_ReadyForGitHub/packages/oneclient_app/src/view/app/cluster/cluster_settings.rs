...
use oneclient_cluster::ClusterUpdate;
                ...
                ScrollArea::new()
                    ...
                    .child(
                        settings_row(
                            IconType::Folder,
                            "Thumbnail",
                            "Custom background art for this instance.",
                            Button::new()
                                .secondary()
                                .small()
                                .on_press(move |_| {
                                    spawn(async move {
                                        if let Some(handle) = rfd::AsyncFileDialog::new().pick_file().await {
                                            // In a real implementation this would save to DB/Local Path
                                            tracing::info!("Picked thumbnail: {:?}", handle.path());
                                        }
                                    });
                                })
                                .text("Change…")
...
impl Component for NameRow {
    fn render(&self) -> impl IntoElement {
        ...
        let mut last = use_state(|| String::new());

        // Initialize last to the initial value
        use_hook(|| last.set(state.read().clone()));

        {
            let dispatch = dispatch.clone();
            use_side_effect(move || {
                let v = state.read().clone();
                if v == *last.peek() {
                    return;
                }
                last.set(v.clone());
                if !v.is_empty() {
                    dispatch.update_cluster(
                        cluster_id,
                        ClusterUpdate {
                            name: Some(v),
                            ..Default::default()
