...
struct PendingRemove {
    path: String,
    /// Whether the files live in PulseClient's own java dir, which is the only
...
fn confirm_remove_modal(
    ...
    let consequence = if managed {
        "This runtime is managed by PulseClient. It's files WILL be deleted."
    } else {
        "This runtime is not managed by PulseClient. It's files will not be deleted."
    };
