...
impl Component for SettingsLauncher {
    fn render(&self) -> impl IntoElement {
        ...
        let gui_scale = use_state({
            let v = settings.gui_scale;
            move || v
        });
        ...
        let consent_summary = if settings.declined_tos {
            "Declined. Poly+ and crash reporting stay off until you accept and restart PulseClient."
        } else {
        ...
        let scale_btn = |scale_val: u32, current_scale: u32, mut setter: State<u32>| {
            let mut btn = Button::new().small().on_press(move |_| setter.set(scale_val)).text(scale_val.to_string());
            if scale_val == current_scale {
                btn = btn.primary();
            } else {
                btn = btn.secondary();
            }
        ...
        let current_scale = *gui_scale.read();
        ...
        settings_page()
            ...
            .child(settings_row(
                IconType::Settings01,
                "GUI Scale",
                "Adjust the size of the application interface.",
                rect()
                    .horizontal()
                    .spacing(8.)
                    .child(scale_btn(1, current_scale, gui_scale.clone()))
                    .child(scale_btn(2, current_scale, gui_scale.clone()))
                    .child(scale_btn(3, current_scale, gui_scale.clone()))
                    .child(scale_btn(4, current_scale, gui_scale.clone()))
            ))
            .child(settings_row(
                IconType::Link03,
                "Discord RPC",
...
fn browse(
    ...
    spawn(async move {
        let mut dialog =
            rfd::AsyncFileDialog::new().set_title("Choose where PulseClient should store game data");
            ...
            label()
                .text(
                    "Your settings and sign-in stay where they are. The old copy is kept until \
                     you remove it, and PulseClient has to restart before it uses the new folder. \
