...
fn copying(plan: &RelocationPlan, copied: u64, total: u64) -> Element {
    ...
    screen()
        ...
        .child(note(
            "Leave PulseClient open until this finishes. Nothing is removed from the old folder, \
...
fn moved(outcome: &RelocationOutcome, actions: Actions) -> Element {
    let mut body = format!(
        "{} copied to {}. PulseClient keeps running from the old folder until you restart it.",
...
fn failed(message: String, actions: Actions) -> Element {
    screen()
        ...
        .child(
            label()
                .text("Nothing was changed. PulseClient is still running from its current folder.")
                .font_size(12.)
                .max_lines(2)
                .color(colors::fg_secondary()),
        )
