...
static LAUNCH_TASKS: LazyLock<Mutex<HashMap<ClusterId, tokio::sync::oneshot::Sender<()>>>> = LazyLock::new(|| Mutex::new(HashMap::new()));
...
impl Actions {
    ...
    pub fn accept_tos(&self, terms_version: u32, privacy_version: u32) {
        ...
        if was_declined {
            self.notify("Restart to finish")
                .body(
                    "Thanks. PulseClient reconnects to Polyfrost services the next time you start \
                     it.",
                )
    ...
    pub fn update_cluster(&self, cluster_id: ClusterId, update: ClusterUpdate) {
        spawn_forever(async move {
            let Ok(state) = launcher::state() else { return };
            match state.clusters.update(cluster_id, update).await {
                Ok(_) => super::invalidate_cluster_queries().await,
                Err(err) => tracing::error!("failed to update cluster: {err:#}"),
            }
        });
    }
    ...
    pub fn launch_cluster(&self, cluster_id: ClusterId) {
        ...
        spawn_forever(async move {
            ...
            tokio::select! {
                _ = launch(&actions, cluster_id) => {
                    if let Some(remaining) = LAUNCH_HOLD.checked_sub(started.elapsed()) {
                        tokio::time::sleep(remaining).await;
                    }
                }
                _ = &mut rx => {
                    tracing::debug!(cluster_id, "launch aborted via cancel button");
                }
            }
            ...
            if let Ok(mut map) = LAUNCH_TASKS.lock() {
                map.remove(&cluster_id);
            }
        });
        
        if let Ok(mut map) = LAUNCH_TASKS.lock() {
            map.insert(cluster_id, tx);
        }
    }

    pub fn browse_modpacks(&self) {
        let _ = open::that("https://modrinth.com/modpacks");
    }

    pub fn import_modpack(&self) {
        let events = self.events.clone();
        spawn_forever(async move {
            if let Some(handle) = rfd::AsyncFileDialog::new()
                .set_title("Select a Modpack (.mrpack)")
                ...
                .await
            {
                ...
                match oneclient_content::bundles::polymrpack::read_meta_from_archive(&path).await {
                    Ok(meta) => {
                        use oneclient_cluster::CreateClusterOptions;
                        let options = CreateClusterOptions::new(meta.name, meta.mc_version, meta.loader);
                        let options = if !meta.loader_version.is_empty() {
                            options.loader_version(meta.loader_version)
                        } else {
                            options
                        };
                        ...
                        match state.clusters.create(&global, options).await {
                            Ok(cluster) => {
                                ...
                                match oneclient_content::packages::modpack::MrpackInstaller::install_archive(path, cluster.id, &content).await {
                                    Ok(_) => {
                                        let _ = state.clusters.set_stage(cluster.id, oneclient_cluster::ClusterStage::NotReady).await;
                                        events.notify("Modpack Imported").body(format!("{} imported! Click Launch to download files and play.", cluster.name)).send();
                                        super::invalidate_cluster_queries().await;
                                    },
                                    Err(e) => {
                                        events.notify("Modpack Import Failed").body(e.to_string()).error().send();
                                    }
                                }
                            },
                            Err(e) => {
                                events.notify("Cluster Creation Failed").body(e.to_string()).error().send();
                            }
                        }
                    },
                    Err(e) => {
                        events.notify("Invalid Modpack").body(e.to_string()).error().send();
                    }
                }
            }
        });
    ...
    pub fn create_custom_cluster(&self, name: String, version: String, loader: oneclient_common::domain::GameLoader) {
        let events = self.events.clone();
        spawn_forever(async move {
            let Ok(state) = launcher::state() else { return };
            use oneclient_cluster::CreateClusterOptions;
            let options = CreateClusterOptions::new(name, version, loader);
            let global = state.settings.read().global_game_settings.clone();
            match state.clusters.create(&global, options).await {
                Ok(cluster) => {
                    events.notify("Instance Created").body(format!("{} is ready", cluster.name)).send();
                    super::invalidate_cluster_queries().await;
                },
                Err(e) => {
                    events.notify("Creation Failed").body(e.to_string()).error().send();
                }
            }
        });
    }

    pub fn delete_cluster(&self, cluster_id: ClusterId) {
        let events = self.events.clone();
        spawn_forever(async move {
            let Ok(state) = launcher::state() else { return };
            // Note: Since I don't have the exact API, I will just call state.clusters.delete
            match state.clusters.delete(cluster_id, true).await {
                Ok(_) => {
                    events.notify("Instance Deleted").send();
                    super::invalidate_cluster_queries().await;
                },
                Err(e) => {
                    events.notify("Deletion Failed").body(e.to_string()).error().send();
                }
            }
        });
    }
    ...
    pub fn kill_cluster(&self, cluster_id: ClusterId) {
        if let Ok(mut map) = LAUNCH_TASKS.lock() {
            if let Some(tx) = map.remove(&cluster_id) {
                let _ = tx.send(());
                self.station
                    .clone()
                    .write_channel(AppChannel::Game)
                    .game
                    .finish_launch(cluster_id);
                // Also reset stage just in case
                if let Ok(state) = launcher::state() {
                    state.games.set_stage(cluster_id, oneclient_events::LaunchStage::Checking);
                }
            }
        }
        ...
        let state = match launcher::state() {
            ...
            Err(err) => {
                ...
                self.write_relocation(RelocationState {
                    plan: Some(plan),
                    result: Some(Err(format!("PulseClient is not ready yet: {err}"))),
