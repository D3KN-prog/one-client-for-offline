...
fn update_prompt(version: &str) -> Prompt<UpdateAnswer> {
    Prompt::new(
        "Update available",
        format!("PulseClient {version} is ready to install. Download and install it now?"),
    )
    .option(
...
async fn run_simulated_update() -> anyhow::Result<()> {
    ...
    let label = format!("Downloading PulseClient {FAKE_VERSION}");
    ...
    events.finish_progress(
        ...
        format!("PulseClient {FAKE_VERSION} is ready. Restart to apply."),
    );
...
async fn run_check(auto_install: bool, events: EventBus) -> anyhow::Result<()> {
    ...
    if !can_self_update() {
        ...
        events
            ...
            .body(format!(
                "PulseClient {} is available. Download the latest package from {} to update.",
...
async fn download_and_install(update: Update, events: EventBus) -> anyhow::Result<()> {
    ...
    let label = format!("Downloading PulseClient {version}");
    ...
    tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
        ...
        events.finish_progress(
            progress_id,
            "Finished Downloading",
            format!("PulseClient {version} is ready. Restart to apply."),
        );
        Ok(())
    })
    .await??;
