        ...
        use_side_effect_with_deps(&self.parallax_enabled, move |enabled| {
            if *enabled {
                let handle = spawn(async move {
                    ...
                    loop {
                        tick += 0.02;
                        let drift_x = (tick * 0.35).sin() * 0.15;
                        let drift_y = (tick * 0.22).cos() * 0.10;

                        let (tx, ty) = *target.peek();
                        let desired_x = tx + drift_x;
                        let desired_y = ty + drift_y;
                        let (cx, cy) = *current.peek();

                        if (desired_x - cx).abs() > SETTLE || (desired_y - cy).abs() > SETTLE {
                            current.set((cx + (desired_x - cx) * STEP, cy + (desired_y - cy) * STEP));
