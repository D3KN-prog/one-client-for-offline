...
impl Component for ClusterShell {
    fn render(&self) -> impl IntoElement {
        ...
        let header = cluster.as_ref().map(|cluster| {
            ...
            let target_folder = cluster.game_dir().ok().map(|base| {
                match active_tab {
                    ClusterViewShellTab::Mods => base.join("mods"),
                    ClusterViewShellTab::Shaders => base.join("shaderpacks"),
                    ClusterViewShellTab::Textures => base.join("resourcepacks"),
                    ClusterViewShellTab::Screenshots => base.join("screenshots"),
                    ClusterViewShellTab::GameLog => base.join("logs"),
                    _ => base,
                }
            });
            cluster_header(
