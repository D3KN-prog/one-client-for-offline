    ...
    fn render(&self) -> impl IntoElement {
        ...
        let on_toggle: EventHandler<()> = {
            ...
            (move |()| {
                ...
                if hash.is_none() && bundle_name.is_none() {
                    let c_id = cluster_id;
                    let f_name = file_name.clone();
                    let target_en = !enabled_now;
                    spawn(async move {
                        if let Ok(state) = crate::launcher::state() {
                            if let Ok(Some(row)) = oneclient_db::dao::cluster::get_by_id(&state.services.db, c_id).await {
                                if target_en {
                                    oneclient_content::packages::store::link::try_move_from_disabled(&row, oneclient_content::packages::ContentType::Mod, &f_name).await;
                                } else {
                                    oneclient_content::packages::store::link::try_move_to_disabled(&row, oneclient_content::packages::ContentType::Mod, &f_name).await;
                                }
                                let _ = crate::hooks::invalidate_cluster_queries().await;
                            }
                        }
                    });
                }
